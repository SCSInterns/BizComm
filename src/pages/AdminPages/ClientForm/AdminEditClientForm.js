...
const AdminEditClientForm = ({ onClose, fetchClients, form }) => {
	const [fields, setFields] = useState([]);

	useEffect(() => {
		if (Object.keys(form.additional_fields).length > 0) {
			const fieldNames = Object.keys(form.additional_fields);
			const fieldsArr = fieldNames.map((name) => {
				const fieldObj = {};
				fieldObj.name = name;
				fieldObj.type = form.additional_fields[name].type;
				fieldObj.options = form.additional_fields[name].options;
				return fieldObj;
			});
			setFields(fieldsArr);
		}
	...
	const handleSubmit = (e) => {
		...
		axios
			.put(`/api/v1/client_forms/${form.id}`, formData)
			.then((res) => {
				toast.success(
	...
	return (
		<FormHeaderComponent headerText={"Edit Client Form"} onClose={onClose}>
			<div className="w-full flex sm:flex-row flex-col sm:items-center justify-end pr-1">
				<button
					type="button"
