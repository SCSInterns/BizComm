...
const EditTemplateForm = ({ selectedTemplate, onClose, fetchTemplates }) => {
	...
	const [bodyVariables, setBodyVariables] = useState(
		selectedTemplate?.content_variables
	);
	...
	const handleFormSubmit = async (values) => {
		...
		formData.append(
			"user_template[template_language]",
			values.template_language
		);
		formData.append(
			"user_template[template_category]",
			values.template_category
		);
		formData.append("user_template[media]", media);
		formData.append("user_template[subtitle]", values.subtitle);
		// formData.append(
		// 	"user_template[buttons]",
		// 	JSON.stringify(values.buttons)
		// );
		// formData.append(
		// 	"user_template[has_buttons]",
		...
		formData.append(
			"user_template[content_variables]",
			JSON.stringify(bodyVariables)
		);

		axios
			.put(`/api/v1/user_templates/${selectedTemplate.id}`, formData, {
				multipart: true,
			})
			.then((response) => {
				toast.success(
		...
		<FormHeaderComponent
			...
			<Formik
				initialValues={{
					...
					template_language: selectedTemplate.template_language || "",
				...
				{({
					...
					<Form className="w-full flex flex-col items-start justify-center gap-4">
						...
						<button className="bg-primary text-white font-normal px-4 py-2.5 rounded-lg">
							Submit
						</button>
					</Form>
				)}
