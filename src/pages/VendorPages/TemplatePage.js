...
const TemplatePage = () => {
	...
	const handleDeleteTemplate = async () => {
		if (selectedTemplate) {
			await axios
				.destroy(`/api/v1/user_templates/${selectedTemplate.id}`)
				.then((res) => {
					toast.success(
						res.data.message || "Template deleted successfully"
					);
					fetchTemplates();
				})
				.catch((error) => {
	...
	return (
		<div className="w-full h-full min-h-[calc(100vh-3rem)] pl-4 mt-3 relative">
			<div className="w-full flex flex-col items-center justify-center md:gap-6 gap-4">
					...
					<div className="h-full flex items-center justify-center gap-2">
						<div className="relative group/add-template">
							<div
								...
								<PlusIcon
									size={13}
									className="absolute right-[5.5px] top-3"
								/>
							</div>
							<CustomTooltip
								text={"Add Template"}
								position={"left"}
								classes={
									"scale-0 group-hover/add-template:scale-100"
								}
							/>
						</div>
					</div>
					...
					<div className="flex items-center justify-center gap-2 relative">
						...
						<div
							...
							{filters.length !== 4 ? (
								...
								<p className="w-full text-center font-normal text-sm">
				...
				{openAddTemplateForm && (
					<AddTemplateForm
						onClose={() => setOpenAddTemplateForm(false)}
